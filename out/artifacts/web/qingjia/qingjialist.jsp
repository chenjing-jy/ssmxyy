                ...
                <table width="100%" height="25" border="0" cellpadding="0" cellspacing="0">
                    <tr>
                        <td align="left" valign="middle" class="bgtop"><font color=""><b>请假列表</b></font>
                            ...
                            请假时间<input type="text" name="qjsj"  onclick="new Calendar().show(this);" value="">
                ...
                <TABLE cellSpacing=0 cellPadding=3 width="100%" align=center border=1 bordercolor="#33CCCC" id="table1">
                    ...
                    <c:forEach items="${list}" var="mymap" varStatus="status">
                        <tr align='center' bgcolor='#FFFFFF' onmouseover='this.style.background="#F2FDFF"'
                            ...
                            <td align="center" height="28">
                                <c:choose>
                                    <c:when test="${mymap.zt eq '同意'}">
                                        <span class="checkzt" style="background-color: #00be00">${mymap.zt}</span>
                                    </c:when>
                                    <c:when test="${mymap.zt eq '不同意'}">
                                        <span class="checkzt" style="background-color: red">${mymap.zt}</span>
                                    </c:when>
                                    <c:otherwise>
                                        <span class="checkzt" style="background-color: #ff970a">${mymap.zt}</span>
                                    </c:otherwise>
                                </c:choose>
                            ...
                            <td>
                                <c:choose>
                                    <c:when test="${mymap.zt == '申请'}">
                                        <del title="请先处理完申请后再删除！" style="color: #868686" class="noDel">删除</del>
                                        |
                                    </c:when>
                                    <c:otherwise>
                                        <a href="<%= request.getContextPath()%>/qingjia/del?keyid=${mymap.qjid}"
                                           onClick="javascript:if(confirm('是否删除')){return true;} else{return false;}">删除</a>|
                                    </c:otherwise>
                                </c:choose>

                                <c:if test="${mymap.zt=='申请'}">

                                    <a href="<%= request.getContextPath()%>/qingjia/modifypage?keyid=${mymap.qjid}">审核</a>|
                                </c:if>
